(function() {
    ...
    function createUI() {
      ...
      container.innerHTML = `
        <div class="gemini-forms-header">
          <img src="${chrome.runtime.getURL('images/icon48.png')}" alt="Gemini Forms Helper">
    ...
    function extractFormData() {
      const formData = {
        title: document.querySelector('.freebirdFormviewerViewHeaderTitle, .F9yp7e')?.textContent || 'Google Form',
        questions: []
      };
      
      // Get all question containers - try different selectors used by Google Forms
      const questionContainers = document.querySelectorAll('.freebirdFormviewerViewNumberedItemContainer, .Qr7Oae[role="listitem"]');
      
      questionContainers.forEach((container) => {
        // Get question text - try different possible selectors
        const questionText = container.querySelector('.freebirdFormviewerViewItemsItemItemTitle, .HoXoMd, .M7eMe')?.textContent || '';
        ...
        if (container.querySelector('[role="radiogroup"], .lLfZXe[role="radiogroup"]')) {
          questionType = 'multipleChoice';
          // Try different option selectors
          const optionElements = container.querySelectorAll('.docssharedWizToggleLabeledLabelWrapper, .aDTYNe, .ulDsOb span');
          optionElements.forEach((option) => {
            const optionElement = option.closest('.freebirdFormviewerViewItemsRadioOptionContainer, .nWQGrd, .docssharedWizToggleLabeledContainer');
            if (optionElement) {
              options.push({
        ...
        // Checkboxes
        else if (container.querySelector('.freebirdFormviewerViewItemsCheckboxContainer, [role="group"]')) {
          questionType = 'checkboxes';
          const optionElements = container.querySelectorAll('.docssharedWizToggleLabeledLabelWrapper, .aDTYNe, .ulDsOb span');
          optionElements.forEach((option) => {
            const optionElement = option.closest('.freebirdFormviewerViewItemsCheckboxOptionContainer, .nWQGrd, .docssharedWizToggleLabeledContainer');
            if (optionElement) {
              options.push({
                text: option.textContent.trim(),
                element: optionElement
        ...
        if (question.type === 'multipleChoice') {
          ...
          if (optionMatch) {
            ...
            if (question.options[optionNum]) {
              ...
              console.log(`Selected option ${optionNum + 1} for question ${questionNum}: ${question.text}`);
            }
          }
          ...
          optionMatches.forEach(optMatch => {
            ...
            if (question.options[optionNum]) {
              ...
              // Log the selection for verification
              console.log(`Selected checkbox option ${optionNum + 1} for question ${questionNum}: ${question.text}`);
            }
          });
        } else if (question.type === 'shortAnswer' || question.type === 'paragraph') {
          // For text inputs
          const input = question.container.querySelector('input[type="text"], textarea');
          if (input) {
            input.value = answer;
            ...
            console.log(`Entered text for question ${questionNum}: ${question.text}`);
          }
        }
      });
